...
#define TFT_DP_Lo PORT
#define TFT_DP_Hi PORT
#define TFT_WR_1 (PORT |= (1<<P))
#define TFT_WR_0 (PORT &= ~(1<<P))
#define TFT_RD_1 (PORT |= (1<<P))
#define TFT_RD_0 (PORT &= ~(1<<P))
#define TFT_CS_1 (PORT |= (1<<P))
#define TFT_CS_0 (PORT &= ~(1<<P))
#define TFT_RS_1 (PORT |= (1<<P))
#define TFT_RS_0 (PORT &= ~(1<<P))
#define TFT_RST_1 (PORT |= (1<<P))
#define TFT_RST_0 (PORT &= ~(1<<P))
...
void Write_Command(uint16_t Wcommand)
{
	...
	LSByte = (uint8_t) (Wcommand & 0x00FF);
	MSByte = (uint8_t) ((Wcommand & 0xFF00)>>8);
...
{
		uint8_t MSByte,LSByte;
		LSByte = (uint8_t) (Wdata & 0x00FF);
		MSByte = (uint8_t) ((Wdata & 0xFF00)>>8);
...
void TFT_Char(char C,unsigned int x,unsigned int y,char DimFont,unsigned int Fcolor,unsigned int Bcolor)
{
	...
	if(DimFont == 8)
	{
		PtrFont = FONT_8x8; // aman & can be eliminated ;;; default prev line: PtrFont = &FONT_8x8;
		Cptrfont = (C-32)*8;
